...
public class State {
    ...
    private List<Pacman>
        _allPlayers = new List<Pacman>(); // list of all players, including dead ones, sorted by initial turn order
    ...
    public void InitializeForTurn(int myScore, int opponentScore, Pacman[] pacs, Pellet[] pellets) {
        // to capture the new information

        _combatScoreThisTurn =
            0; // probably not necessary as this is usually only read and set in fictional states, but good practice just in case
            ...
            if (opponent.Location == subject.Location) {
                ...
                scoreDelta /=
                    (subject.IsOurPlayer
                        ? MyPacs.Count
                        : Enemies
                            .Count); // the more we kill, the more valuable later kills are. losing your last pac is a lot worse than losing your first
    ...
    public List<GameAction> GetMoves() {
        ...
        if (turnPac.AbilityCooldown == 0) {
            ...
            kids.Add(new GameAction(turnPac.Location, ActionType.Switch,
                                    SwitchPac.SwitchOptions(turnPac.Type, "PREY")));
            kids.Add(new GameAction(turnPac.Location, ActionType.Switch,
                                    SwitchPac.SwitchOptions(turnPac.Type, "PREDATOR")));
        }
    ...
    private int FloodFill() {
        ...
        HashSet<Point>[]
            cellsToUpdateSet = new HashSet<Point>[_turnOrder.Count]; // used to check for duplicates in queue
    ...
    private bool FloodFillHelper(Pacman[] turnOrderCopy, Queue<Point>[] cellsToUpdate,
                                 HashSet<Point>[] cellsToUpdateSet,
